...
SELECT * FROM PatientTable WHERE pat_id=?;
...
SELECT * FROM RecordsTable WHERE pat_id=?;

-- Create Prescription (get pat_id from selection)
INSERT INTO PrescriptionTable(med_name,rx_start,rx_end,rx_desc,pat_id)
VALUES(?,?,?,?,?);
...
BEGIN
    ...
    ELSE
        INSERT INTO PrescriptionTable(med_name,rx_start,rx_end,rx_desc,pat_id)
        VALUES(NEW.med_name,NEW.rx_start,NEW.rx_end,NEW.rx_desc,NEW.pat_id)
    END IF
END;

-- List appointments (get pat_id from cookies)
SELECT * FROM AppointmentTable WHREE doc_id=?;
...
-- List appointments (get pat_id from cookies)
SELECT app_time,LocationTable.loc_name,StaffTable.staff_name 
